...
def enter_move(board):
	...
	while not ok:
		...
		if not ok:	# it's occupied - to the input again
			...
			continue
	board[row][col] = 'Y' 	# set '0' at the selected square
...
def make_list_of_free_fields(board):
	...
	for row in range(3): # iterate through rows
		for col in range(3): # iterate through columns
			if board[row][col] not in ['Y','C']: # is the cell free?
				free.append((row,col)) # yes, it is - append new tuple to the list
	return free


def victory_for(board,sgn):
	if sgn == "C":	# are we looking for X?
...
board[1][1] = 'C' # set first 'X' in the middle
...
while len(free):
	...
	if human_turn:
		enter_move(board)
		victor = victory_for(board,'Y')
	else:	
		draw_move(board)
		victor = victory_for(board,'C')
	if victor != None:
		break
